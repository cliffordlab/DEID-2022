...
The current script focuses on the PHI de-identification of YearDate from attributes of the medical free texts.
The two following functions (check_for_DateYear, deid_DateYear) extract and reveal the location of the existing 
YearDates from the available texts.

How to run the code?
    python deid_YearDate_AlirezaRafiei.py <input_filename> <output_filename>
    # input_filename (.txt): Free medical text (e.g., id.txt)
    # output_filename (.phi): Output file with patient name records and the extracted DateYaer (e.g., DateYaer.phi)
    
How to evaluate the results?
    The performance of the script can be evaluated using the stats.py code (presented in this repo)
    and the following command
    python stats.py id.deid id-phi.phrase DateYear_AlirezaRafiei.phi
...
DateYear_pattern = "([^>/@-](19|20)\d{2}[^-Cc+](?![0-9]))|([']\d{2})|(([^>-])\d{2}['][^sS])|([^>-](?![0-9])[5-9][0-9](?![0-9])([^-%MGmg/cC]))|(/00/)"
# DateYear patterns can be presented in several different ways. "19xx, 20xx", "'xx", "xx'" are the most comment types of DateYear patterns.
# Since the defined pattern aims to extract DateYears, every sign, including characters and units, that represents other measurements
# should be excluded. Also, the DateYear patterns are either two or four digits, including 00, and the consideration of "'s" is important
# in the detection of DateYear PHI.


# compiling the reg_ex would save sime time!
dy_reg = re.compile(DateYear_pattern)
...
def check_for_DateYear(patient, note, chunk, output_handle):
    ...
    Inputs:
        ...
        - output_handle: an opened file handle. The results will be written to this file.
            ...
            to this function. 
    Logic:
        Search the entire chunk for DateYear occurances. Find the location of these occurances 
        ...
        Use the precompiled regular expression to find DateYears.
    """
    ...
    for match in dy_reg.finditer(chunk):
...
def deid_DateYear(text_path= 'id.text', output_path = 'DateYear_AlirezaRafiei.phi'):
    """
    Inputs: 
    ...
    Outputs:
        for each patient note, the output file will start by a line declaring the note in the format of:
            Patient X Note Y
        then for each DateYear found, it will have another line in the format of:
            start start end
        where the start is the start position of the detected DateYear string, and end is the detected
        ...
        to the output
    Screen Display:
        For each DateYear detected, the following information will be displayed on the screen for debugging purposes 
        (these will not be written to the output file):
            start end DateYear
        where `start` is the start position of the detected DateYear string, and `end` is the detected end position of the string
    ...
    with open(output_path,'w+') as output_file:
        with open(text_path) as text:
            ...
            for line in text:
                ...
                if len(record_end):
                    # Now we have a full patient note stored in `chunk`, along with patient number and note DateYear
                    # pass all to check_for_DateYear to find any DateYears in note.
